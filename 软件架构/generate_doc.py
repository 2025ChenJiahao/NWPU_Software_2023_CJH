...
# 2.2 用例图
add_title('2.2 用例图', 2)
add_para('以下为系统的用例图（PlantUML语法描述，可复制至PlantUML工具渲染）：')

plantuml_uc = """@startuml
...
rectangle "共享自习室预约管理系统" {
    usecase "注册/登录" as UC1
    usecase "浏览自习室" as UC2
    usecase "查看时段座位状态" as UC3
    usecase "预约座位" as UC4
    usecase "支付订单" as UC5
    usecase "扫码签到" as UC6
    usecase "查看预约记录" as UC7
    usecase "取消预约" as UC8
    usecase "管理自习室信息" as UC9
    usecase "查看所有订单" as UC10
    usecase "标记异常" as UC11
}

actor "用户账户系统" as AuthSys
actor "支付系统" as PaySys
actor "门禁控制系统" as AccessSys
...
预约管理系统 --> 用户账户系统 : 身份认证接口
预约管理系统 --> 支付系统 : 支付/退款接口
预约管理系统 --> 门禁控制系统 : 入场验证接口

@enduml"""

p = doc.add_paragraph()
run = p.add_run(plantuml_ctx)
run.font.name = 'Consolas'
run.font.size = Pt(8)
...
User "1" -- "*" Reservation : 创建 >
StudyRoom "1" -- "*" Seat : 包含 >
StudyRoom "1" -- "*" TimeSlot : 提供 >
Reservation "*" -- "1" Seat : 预约 >
Reservation "*" -- "1" TimeSlot : 属于 >
Reservation "1" -- "0..1" Order : 关联 >
Reservation "1" -- "*" Violation : 可能有 >
User "1" -- "*" Order : 持有 >

@enduml"""

p = doc.add_paragraph()
run = p.add_run(plantuml_class)
run.font.name = 'Consolas'
run.font.size = Pt(8)
...
plantuml_comm = """@startuml
skinparam sequenceMessageAlign center

actor "普通用户" as User
participant "SeatMapPage" as SMP
...
User -> SMP : 1. 选择自习室和时段
SMP -> RC : 2. requestSeatMap(roomId, slotId)
RC -> AC : 3. checkAvailability(roomId, slotId)
AC -> Seat : 4. querySeats(roomId, slotId)
Seat --> AC : 5. 返回座位状态列表
AC --> RC : 6. 返回可用座位
RC --> SMP : 7. 展示座位状态图

User -> SMP : 8. 选择座位(seatId)
SMP -> RC : 9. reserveSeat(userId, seatId, slotId)
RC -> AC : 10. checkUserConflict(userId, slotId)
AC --> RC : 11. 无冲突

RC -> Seat : 12. lockSeat(seatId, 5min)
Seat --> RC : 13. 锁定成功

RC -> Res : 14. createReservation(userId, seatId, slotId)
Res --> RC : 15. reservationId

RC -> Order : 16. createOrder(reservationId, amount)
Order --> RC : 17. orderId

RC --> SMP : 18. 返回订单信息
SMP -> PC : 19. initiatePayment(orderId)
PC -> PG : 20. createPayRequest(orderId, amount)
PG --> PC : 21. 返回支付页面/二维码
PC --> User : 22. 展示支付页面

note over Res, Seat : 5分钟内未支付将自动取消

@enduml"""

p = doc.add_paragraph()
run = p.add_run(plantuml_comm)
run.font.name = 'Consolas'
run.font.size = Pt(8)
...
PendingPayment --> Expired : 支付超时(5min)
PendingPayment --> Cancelled : 用户取消

Confirmed --> InUse : 用户签到
Confirmed --> Cancelled : 用户取消(开始前)
Confirmed --> NoShow : 时段结束未签到

InUse --> Completed : 时段结束/用户离开
...
add_table(
    ...
    [
        ...
        ['PendingPayment', '5分钟超时', 'Expird', '释放座位，取消订单'],
...
# ========== 保存 ==========
output_path = os.path.join(os.path.dirname(__file__), '共享自习室预约管理系统_建模作业.docx')
